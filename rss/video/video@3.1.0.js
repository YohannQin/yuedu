...
function trace(...args) {
        ...
        java.log('[HTRACE] ' + formatArgs(args));
}

function debug(...args) {
        if (LEVELS[LOG_LEVEL] > LEVELS.debug) return;
        java.log('[HDEBUG] ' + formatArgs(args));
 }

function info(...args) {
        if (LEVELS[LOG_LEVEL] > LEVELS.info) return;
        java.log('[HINFO] ' + formatArgs(args));
}

function error(...args) {
        if (LEVELS[LOG_LEVEL] > LEVELS.error) return;
        java.log('[HERROR] ' + formatArgs(args));
    }
        </script>
	...
	debug('select ', name, data,  data.videos[0])
	//art.reset()
	
	video = data.videos[0]
	art.switchQuality(video.url)
...
var server_data =  temp_video_data
