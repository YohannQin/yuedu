...
<head>
    ...
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/viewerjs/1.11.3/viewer.min.css">
        ...
        <link rel="stylesheet" href="https://unpkg.com/mobile-select@1.4.0/dist/style/mobile-select.css">
...
<body>
    ...
    <script src="https://unpkg.com/mobile-select@1.4.0/dist/mobile-select.umd.js"></script>
    ...
    // 状态机Group
		class SimpleStateMachineGroup {
			...
			exists(name) { return this.map.has(name); }
			getState(name) { return this.map.get(name); }
				...
				imageObserver = new IntersectionObserver((entries) => {
					entries.forEach(entry => {
						...
						if (entry.isIntersecting && rect.height > 200) {
                            ...
                            //if (curr_pic_count - index < 10 && !is_loading) {
                            ...
                            if (last_pic_idx > index && last_pic_idx - index < 10)  {
                                need_loading = true;
                            } else if (total_last_pic_idx - index < 10) {
                                need_loading = true;
                            }
  ...
  return Array.from(elements).map(el => {
    ...
    return el.getAttribute(attrName);
        ...
        async function fetch_next_handle(page, page_url) {
            ...
            if (fetch_page_idx > last_page_idx)
                last_page_idx = fetch_page_idx;
       ...
       // init
        ...
        function page_jump_handle(page) {
            ...
            if (!page_url_handle) {
				...
				java.log('跳转页码：' + page)
				const page_url = page_url_handle(html_href, page);
        ...
        pageInputEl.addEventListener('keydown', function(e) {
            ...
            if (e.key === 'Enter') {
				...
				java.log('跳转页码：' + targetPage)
				const page_url = page_url_handle(html_href, targetPage);
    ...
    var mobileSelect = new MobileSelect({
        trigger: '.page-indicator',
        title: '选择页码',
        wheels: [{ data: numberData }],
        position: 0,
        scrollSpeed: 2,
        triggerDisplayValue: false, // 关键配置：禁止自动更新 trigger 内容
        callback: function (indexArr, data) {
            alert(data);
            //pageInputEl.value = 'aa' + data;
        }
    });
    */
...
document.querySelector('.page-indicator').addEventListener('click', function () {
    		if (page_jump) {
                picker.show();
            }
});
